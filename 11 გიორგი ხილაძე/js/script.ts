// main.ts
const API_KEY: string = "163863b6e313491670b4f52c19c982df";
const DEFAULT_CITY: string = "Batumi";
...
interface ForecastItem extends WeatherApiResponse {
  dateString?: string;
}
...
// ---------------- Global DOM Elements ----------------
let cityInput: HTMLInputElement | null;
let searchButton: HTMLButtonElement | null;
let errorMessageElement: HTMLElement | null;
let loaderWrapper: HTMLElement | null;
let videoSourceElement: HTMLSourceElement | null;
...
// ---------------- Video Background ----------------
function mapWeatherConditionToVideo(weatherMainCondition: string): string {
  const videoMap: Record<string, string> = {
    clear:
      "https://res.cloudinary.com/dijovppjc/video/upload/v1752425423/sunny_rtftgb.mp4",
    clouds:
      "https://res.cloudinary.com/dijovppjc/video/upload/v1752425425/cloudy_p368kg.mp4",
    rain: "https://res.cloudinary.com/dijovppjc/video/upload/v1752425423/rain_v3jxid.mp4",
    drizzle:
      "https://res.cloudinary.com/dijovppjc/video/upload/v1752425423/rain_v3jxid.mp4",
    thunderstorm:
      "https://res.cloudinary.com/dijovppjc/video/upload/v1752425425/thunder_uvciro.mp4",
    ...
    smoke:
      "https://res.cloudinary.com/dijovppjc/video/upload/v1752425423/cyklon_hogksf.mp4",
...
function updateBackgroundVideo(weatherMainCondition: string): void {
...
async function getFiveDayForecast(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `HTTP error getting forecast! ${response.status} - ${response.statusText}: ${errorText}`
      );
    }
...
// ---------------- Helpers ----------------
function getWeatherIconUrl(owmIconCode: string): string {
...
// ---------------- Display Functions ----------------
function displayCurrentWeather(weatherData: WeatherApiResponse | null): void {
  if (!weatherData) return;

  const cityNameElement = document.querySelector<HTMLElement>(".currentCity");
  const temperatureElement = document.querySelector<HTMLElement>(".cityDegree");
  const feelsLikeElement = document.querySelector<HTMLElement>(".feelsLike");
  const windSpeedElement = document.querySelector<HTMLElement>(".windSpeed");
  const humidityElement = document.querySelector<HTMLElement>(".humidity");
  const visibilityElement = document.querySelector<HTMLElement>(".visibility");
  const pressureElement = document.querySelector<HTMLElement>(".pressure");
  const sunriseElement = document.querySelector<HTMLElement>(".sunriseTime");
  const sunsetElement = document.querySelector<HTMLElement>(".sunsetTime");
  const weatherDescriptionElement = document.querySelector<HTMLElement>(
    ".weatherDescription"
  );
  const airQualityElement = document.querySelector<HTMLElement>(".airQuality");
  const uvIndexElement = document.querySelector<HTMLElement>(".uvIndex");
...
function processForecastData(
  ...
  const sortedDailyForecasts = Object.values(dailyForecasts).sort(
    (a, b) => a.dt - b.dt
  );
...
function getRandomizedTime(
  ...
  const newDate = new Date(
    baseDate.getTime() + sign * randomOffset * 60 * 1000
  );
...
function displayFiveDayForecast(
  ...
  const forecastContainer =
    document.querySelector<HTMLElement>(".week-weather");
...
// ---------------- Main ----------------
async function loadWeatherForCity(cityName: string): Promise<void> {
  displayErrorMessage("");
  showLoader();
  ...
  try {
    ...
    console.error(
      "Critical Error during data load in loadWeatherForCity:",
      error
    );
    displayErrorMessage(
      error.message || "An unexpected error occurred during data loading."
    );
  } finally {
    hideLoader();
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  videoElement = document.querySelector<HTMLVideoElement>(
    ".video-background video"
  );
  videoSourceElement = document.querySelector<HTMLSourceElement>(
