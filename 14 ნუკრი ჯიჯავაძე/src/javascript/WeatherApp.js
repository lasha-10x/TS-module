...
export class WeatherApp {
    ...
    async fetchWeather(city) {
        const currentRes = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${this.apiKey}&units=metric`);
        if (!currentRes.ok)
            throw new Error('City not found');
        const currentData = await currentRes.json();
        const { lat, lon } = currentData.coord;
        const oneCallRes = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely,alerts&appid=${this.apiKey}&units=metric`);
        if (!oneCallRes.ok)
    ...
    async fetchHourlyForecast(lat, lon) {
        if (this.cachedData.oneCallData?.hourly)
            return this.cachedData.oneCallData.hourly;
        const res = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=current,minutely,daily,alerts&appid=${this.apiKey}&units=metric`);
        if (!res.ok)
    ...
    async fetchWeeklyForecast(lat, lon) {
        if (this.cachedData.oneCallData?.daily)
            return this.cachedData.oneCallData.daily;
        const res = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=current,minutely,hourly,alerts&appid=${this.apiKey}&units=metric`);
        if (!res.ok)
    ...
    renderPlaceholder() { }
    renderWeather(data) { }
    animateTransition(renderFn) { return Promise.resolve(); }
    chooseCategory() { }
    loadCategoryData(category, oneCallData) { }
}
