...
export async function fetchWeather(city) {
    ...
    const currentRes = await fetch(`${BASE_URL}/weather?q=${encodeURIComponent(city)}&appid=${API_KEY}&units=metric`);
    if (!currentRes.ok)
    ...
    const daily = Object.values(daysMap)
        .slice(0, 5)
        .map((dayArr) => {
        const closest = dayArr.reduce((prev, curr) => Math.abs(curr.hour - 12) < Math.abs(prev.hour - 12) ? curr : prev);
        return {
            day: closest.day,
            temp: closest.temp,
            icon: closest.icon,
