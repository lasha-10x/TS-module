// utils.ts
export const KelvinToCelsius = (k) => `${Math.round(k - 273.15)}°C`;
export const KelvinToFahrenheit = (k) => `${Math.round(((k - 273.15) * 9) / 5 + 32)}°F`;
export const calculateVisibility = (m) => `${(m / 1000).toFixed(1)} km`;
...
const COUNTRY_CODE = document.querySelector(".country");
const temp = document.querySelector(".temp");
const WEATHER_INFO = document.querySelector(".weather-info");
const HUMIDITY_INDEX = document.querySelector(".humidity");
const WIND_SPEED_INDEX = document.querySelector(".wind");
const PRESSURE_INDEX = document.querySelector(".pressure");
const FEELS_LIKE_INDEX = document.querySelector(".feels-like");
const VISIBILITY_INDEX = document.querySelector(".visibility");
const SUNRISE_TIME = document.querySelector(".sunrise");
const SUNSET_TIME = document.querySelector(".sunset");
const MAX_TEMPERATURE = document.querySelectorAll(".max-temp");
const MIN_TEMPERATURE = document.querySelectorAll(".min-temp");
const DAY_WEATHER = document.querySelectorAll(".day-weather");
const days = [
    "Sunday",
    "Monday",
    "Tuesday",
...
const renderTodayWeather = (cities) => {
    ...
    if (mainWeatherIcon)
        mainWeatherIcon.src = weatherIcon;
};
const renderForecast = (cities) => {
    ...
    dailyForecasts.forEach((dayForecast, index) => {
        ...
        const dayNameEl = document.querySelectorAll(".days .day")[index];
        if (maxTempEl)
        ...
        if (dayWeatherEl)
            dayWeatherEl.textContent = dayForecast.weather;
        if (dayNameEl)
            dayNameEl.textContent = dayForecast.dayName;
    });
};
